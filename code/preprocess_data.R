...
#delete thos entries with 0 counts across all samples
...
# 
# p <- ggplot(counts2plot, aes(x=batch, y=cpm)) +
#   geom_boxplot() + theme_minimal() + scale_fill_wa_d(wacolors$volcano)#+ ylim(c(0,100))   #+
# file_name <- paste(resultsPath,"/ROSMAP_counts_boxplot.pdf",sep="")
# ggsave(file_name, p, width = 12, height = 10, units = "cm",dpi = 400)
...
#   }
# }
norm_matrix  <-counts_matrix/colSums(counts_matrix)
meanSEQdepth <- mean(colSums(counts_matrix))
norm_matrix  <- norm_matrix*meanSEQdepth
...
sample_ID[which(grepl('No_AD_',sample_ID))] <- 'No_AD'
...
#autoplot(pca_object, data= t_c_matrix, colour = 'batch')
#remove two oulier samples (PC2) from batch 5
x    <- pca_object$x
...
#
#remove batch effect-
adjusted <- ComBat_seq(as.matrix(counts_matrix.processed), batch=annotation$Batch, group=NULL)
